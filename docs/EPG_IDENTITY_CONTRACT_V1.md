...
7. Schedule overlap is a registry validation problem, not an identity collision rule. The current registry preserves canonical identity and does not silently select a winner; explicit source-priority conflict resolution remains a follow-on adapter/merge boundary.

...
interface ArchiveMetadataResponse {
  ...
  files?: ArchiveFile[];
  // Archive.org metadata responses commonly expose the actual serving node
  // and item directory. Prefer these when present so playback uses the same
  // canonical CDN form proven by the isolated native <video> test.
  server?: string;
  d1?: string;
  d2?: string;
  dir?: string;
...
/**
 * Build the actual Archive.org serving URL from verified metadata.
 *
 * Preferred form (when IA supplies server + dir/d1):
 *   https://ia801601.us.archive.org/21/items/IDENTIFIER/file.mp4
 *
 * Fallback remains the verified redirect form:
 *   https://archive.org/download/IDENTIFIER/file.mp4
 *
 * This function NEVER invents the filename; `filename` must come from the
...
function buildFileUrl(
  ...
  const encodedFile = encodeArchivePath(filename);
  const server = String(metadata.server || metadata.d1 || metadata.d2 || "")
    .replace(/^https?:\/\//i, "")
    .replace(/\/+$/, "");
  const dir = String(metadata.dir || "").replace(/^\/+|\/+$/g, "");

  if (server && dir) {
    return `https://${server}/${dir}/${encodedFile}`;
  }
...
/**
 * Add the Archive TV-News server-side clip contract to an already verified
 * media URL. This mirrors the isolated FOX test without changing the media
 * filename or host.
 */
function addTvNewsClip(url: string, identifier: string): string {
  if (!TV_ID_RE.test(identifier)) return url;

  try {
    const u = new URL(url);
    if (!u.searchParams.has("start")) u.searchParams.set("start", "0");
    if (!u.searchParams.has("end")) u.searchParams.set("end", "300");
    if (!u.searchParams.has("ignore")) u.searchParams.set("ignore", "x.mp4");
    if (!u.searchParams.has("cnt")) u.searchParams.set("cnt", "0");
    return u.toString();
  } catch {
    return url;
  }
...
export async function resolveBestFileUrl(identifier: string): Promise<ResolvedFile> {
  // Fast path for TV News archive items: always standard MP4 with 300s slice
  if (TV_ID_RE.test(identifier)) {
    return {
      url: `https://archive.org/download/${identifier}/${identifier}.mp4?start=0&end=300`,
      duration: 3600,
      format: "h.264",
      fallback: false,
    };
  ...
  try {
    ...
    if (mediaFiles.length > 0) {
      ...
      return {
        url: addTvNewsClip(buildFileUrl(identifier, best.name, data), identifier),
...
async function itemsToProgramBlocks(items: TVNewsItem[]): Promise<ScheduleProgram[]> {
  ...
  const resolved = await Promise.all(
    items.map(async (item) => {
      ...
      if (resolvedFile.fallback) {
        return getSafeArchiveUrl(`https://archive.org/download/${item.identifier}/`).replace(
          /^https:\/\/archive\.org/,
          "",
        );
      }
      // The resolved URL is already a verified, browser-playable Archive URL.
      // Keep it intact; do not normalize it back through /download or strip
      // the TV-News clip query that the native playback test proves works.
  ...
  return items.map((item, index) => ({
    ...
    startHour: index * blockHours,
    endHour: (index + 1) * blockHours,
    archivePath: resolved[index],
  }));
}

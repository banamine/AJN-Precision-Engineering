...
function toProxyPath(fullUrl:string){try{const u=new URL(fullUrl);return `/api/archive/proxy?path=${encodeURIComponent(u.pathname+u.search)}`}catch{return fullUrl;}}

async function resolveNewsFile(identifier:string){
 return toProxyPath(getSafeArchiveUrl(`https://archive.org/download/${identifier}/${identifier}.mp4?start=0&end=300`));
}
...
async function itemsToProgramBlocks(items:TVNewsItem[]):Promise<ScheduleProgram[]>{
 ...
 const resolved=await Promise.all(distinct.map(async item=>({item,path:await resolveNewsFile(item.identifier)})));
 let cursor=0;
 return resolved.map(({item,path})=>{
   const duration=Math.max(1,Math.min(24,cursor+item.durationMins/60)-cursor);
   const start=cursor, end=Math.min(24,cursor+duration);
   cursor=end;
   return {title:item.program||item.title||item.identifier,startHour:start,endHour:end,archivePath:path};

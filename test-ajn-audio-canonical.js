...
const later = canonicalizeAjnAudioItems([
  { ...item, url: 'https://cdn.example.test/audio/hour-001.mp3?token=gamma' },
], 'hourly');

assert(later[0].id === first[0].id, 'Repeated audio ingestion must preserve program identity across polling cycles');
assert(
  getCanonicalPrograms().filter(program => program.id === first[0].id).length === 1,
  'Repeated audio ingestion must not duplicate canonical programs'
);

const segment = canonicalizeAjnAudioItems([
  {
    ...item,
    id: 'ajn:segment:https://cdn.example.test/audio/seg-001.mp3',
    url: 'https://cdn.example.test/audio/seg-001.mp3',
    metadata: { ...item.metadata, resourceKind: 'segment' },
  },
], 'segment');

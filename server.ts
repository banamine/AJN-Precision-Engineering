...
function validateArchivePath(raw:string){
 ...
 const cleanPath=cdnMatch?`/download/${cdnMatch[1]}${cdnMatch[2]}`:raw.startsWith('/download/')?raw:null;
 if(!cleanPath)return{valid:false,error:'Only Archive.org /download paths are permitted'};
 return{valid:true,cleanPath};
}
const ARCHIVE_BASE='https://archive.org';
function mediaTypeAllowed(value:string|null){return !!value&&/^(?:video|audio)\//i.test(value.trim());}

app.get('/api/archive/proxy',async(req,res)=>{
 stats.totalRequests++;
 const proxyRequestId=crypto.randomUUID(); const started=Date.now();
 res.setHeader('x-proxy-request-id',proxyRequestId); res.setHeader('Access-Control-Allow-Origin','*');
 ...
 const rawPath=String(req.query.path||''); const v=validateArchivePath(rawPath);
 ...
 try{
   ...
   if(!upstream.ok||!mediaTypeAllowed(contentType)||!upstream.body){
     ...
     console.warn('[Archive Proxy] non-media/unavailable upstream',{proxyRequestId,archivePath:v.cleanPath,archiveUrl:upstreamUrl,finalUrl:upstream.url,status:upstream.status,contentType,range,bodyPreview:bodyPreview.slice(0,300)});
     return res.status(upstream.status>=400?upstream.status:502).json({error:'Archive media unavailable or non-media upstream response',upstreamStatus:upstream.status,contentType,proxyRequestId});
   }
   ...
   res.status(upstream.status); stats.successfulRequests++; stats.activeStreams++;
   const reader=upstream.body.getReader(); let bytesForwarded=0; let clientClosed=false;
   const onClose=()=>{clientClosed=true;void reader.cancel().catch(()=>{});}; res.once('close',onClose);
   try{
     while(true){const {done,value}=await reader.read();if(done||clientClosed)break;const chunk=Buffer.from(value);bytesForwarded+=chunk.length;if(!res.write(chunk))await new Promise<void>(resolve=>res.once('drain',resolve));}
   }finally{
     res.removeListener('close',onClose);try{await reader.cancel();}catch{} if(!res.writableEnded&&!res.destroyed)res.end();
     stats.activeStreams=Math.max(0,stats.activeStreams-1);
     console.log('[Archive Proxy Stream Complete]',{proxyRequestId,status:upstream.status,range:range||'none',declaredLength:upstream.headers.get('content-length')||'unknown',contentRange:upstream.headers.get('content-range')||'none',bytesForwarded});
   }
 }catch(error:any){
   stats.failedRequests++; console.error('[Archive Proxy] transport failure',{proxyRequestId,archivePath:v.cleanPath,archiveUrl:upstreamUrl,error:error?.message||String(error)});
   if(!res.headersSent)return res.status(502).json({error:'Archive proxy transport failure',proxyRequestId}); if(!res.destroyed)res.destroy();
 }
});
...
app.get('/api/archive/metadata',async(req,res)=>{
 ...
 try{const r=await fetch(`${ARCHIVE_BASE}${v.cleanPath}`,{method:'HEAD',redirect:'follow',headers:{'User-Agent':'AJN-Precision-Engineering/1.0'}});res.json({status:r.status,ok:r.ok,contentType:r.headers.get('content-type'),contentLength:r.headers.get('content-length'),acceptRanges:r.headers.get('accept-ranges'),finalUrl:r.url,proxyUrl:`/api/archive/proxy?path=${encodeURIComponent(v.cleanPath)}`});}catch(e:any){res.status(502).json({error:e.message});}
});
